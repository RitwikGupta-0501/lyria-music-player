...
### 🛠️ Fixes & Lifecycle Management
* **Graceful Process Teardown on Window Close**:
  - Registered `on_window_event` handler on `tauri::Builder` intercepting `CloseRequested` on the main window.
  - Fixes terminal execution hang where closing the main window left hidden background webviews (`sandbox-vm`, `/debug`) alive in memory.
  - Ensures clean thread joining for audio and database actor loops and triggers guaranteed process exit.
